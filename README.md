...
<p class="todo">@20220301 → ? → @20220302 → ? → @20220303 → ? → @20220305 → ? → @20220308 → ? → @20220316 → ? ⇒ @20220401 → ? ⇒ </p>
曲：
昨天： 1. 上午311MCA实车100公里，下午实车200公里。验证死机黑屏,未复现。可能操作强度不够，未复现问题。建议后续实车路测挂上monkey，在死机黑屏问题。2. 埋点压测，目前完成了本地媒体的适配，还差小桐FM模块。
今天： 1.小桐FM模块埋点压测适配。2. 下午302实车动态200公里。
弓：
昨天：1. 跑monkey过程中性能培训讲解 2. MTBF用例全部调试了一遍
今天：1. 完成MTBF所有调试。
刘：
昨天：完成所有的MTBF用例开发与调试
今天：调试内存泄漏
王：
昨天：1. 汝燕的性能测试需求脚本，完成了所有性能数据(CPU/MEM)，定期采集,保存，在按退出键后，保存数据为大文件。2. 用研app第一阶段结束，输出测试结果。探讨第二阶段需求。
今天：1. 完成大文件数据（CPU/MEM）数据的解析,生成指定格式excel 
...
gantt
    ...
    1. 跑monkey过程中性能培训讲解 2. MTBF用例全部调试了一遍       : done,   monf  , 2022-02-28 , 1d
